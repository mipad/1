...
namespace Ryujinx.Graphics.Vulkan
{
    unsafe public sealed class VulkanRenderer : IRenderer
    {
        ...
        private volatile bool _surfaceValid;
        ...
        internal static readonly object VulkanObjectCreationLock = new object();

        internal CommandBufferPool BackgroundCommandBufferPool { get; private set; }
        ...
        internal bool IsHiSilicon { get; private set; }
        ...
        private unsafe void LoadFeatures(uint maxQueueCount, uint queueFamilyIndex)
        {
            ...
            if (SupportsASTCDecodeMode)
            {
                ...
                {
                    SType = (StructureType)1000347001,
                };
            ...
            if (SupportsASTCDecodeMode)
            {
                ...
                {
                    SType = (StructureType)1000347000,
                };
            ...
            bool supportsClipDistance = features2.Features.ShaderClipDistance;
            ...
            // ===== 修复 IsMaliGPU：必须在 GpuRenderer 赋值之后再判断 =====
            IsMaliGPU = Vendor == Vendor.ARM || (GpuRenderer?.Contains("Mali") ?? false);
            // ==========================================================

            IsHiSilicon = Vendor == Vendor.HiSilicon;
            ...
            IsFeedbackLoopDevice = IsAmdRdna3 || IsAdreno6xx || IsAdreno7xx || IsMaliGPU || IsHiSilicon;
            ...
            // ===== 同步到 FramebufferParams，启用 Mali 附件补齐 =====
            FramebufferParams.ForceMaliPadTo8 = IsMaliGPU;
            Logger.Info?.Print(LogClass.Gpu, $"[Mali] ForceMaliPadTo8 = {FramebufferParams.ForceMaliPadTo8}");
            // =======================================================
            ...
            Capabilities = new HardwareCapabilities(
                ...
                _physicalDevice.IsDeviceExtensionPresent("VK_EXT_extended_dynamic_state2") && featuresExtendedDynamicState2.ExtendedDynamicState2 && !IsAdreno8xx,
                ...
                maxMemoryBarrierCount: properties.Limits.MaxMemoryAllocationCount);
            ...
            if (BackgroundQueue.Handle != 0)
            {
                BackgroundCommandBufferPool = new CommandBufferPool(
                    Api, _device, BackgroundQueue, BackgroundQueueLock,
                    queueFamilyIndex, IsQualcommProprietary, false);
            }
            ...
            // ===== HelperShader 传入 PhysicalDevice，由它负责创建共享 dummy =====
            HelperShader = new HelperShader(this, _device, _physicalDevice.PhysicalDevice);
            // ===================================================================
        ...
        private void SetupContext(GraphicsDebugLevel logLevel)
        {
            ...
            _initialized = true;
            _surfaceValid = _surface.Handle != 0;
        }
        ...
        public unsafe Capabilities GetCapabilities()
        {
            ...
            return new Capabilities(
                ...
                supportsClipDistance: Capabilities.SupportsClipDistance,
        ...
        private void PrintGpuInformation()
        {
            ...
            // ===== 新增：打印 Mali 是否支持 ClipDistance =====
            Logger.Error?.PrintMsg(LogClass.Gpu,
                $"[Mali] IsMaliGPU={IsMaliGPU}, SupportsClipDistance={Capabilities.SupportsClipDistance}");
            // =================================================

            uint maxGeometryOutputVertices = _physicalDevice.PhysicalDeviceProperties.Limits.MaxGeometryOutputVertices;
            Logger.Warning?.PrintMsg(LogClass.Gpu, $"MaxGeometryOutputVertices = {maxGeometryOutputVertices}");

            var limits = _physicalDevice.PhysicalDeviceProperties.Limits;

            Logger.Warning?.PrintMsg(LogClass.Gpu, $"maxComputeWorkGroupInvocations = {limits.MaxComputeWorkGroupInvocations}");
            Logger.Warning?.PrintMsg(LogClass.Gpu, $"maxComputeWorkGroupSize[0] = {limits.MaxComputeWorkGroupSize[0]}");
            Logger.Warning?.PrintMsg(LogClass.Gpu, $"maxComputeWorkGroupSize[1] = {limits.MaxComputeWorkGroupSize[1]}");
            Logger.Warning?.PrintMsg(LogClass.Gpu, $"maxComputeWorkGroupSize[2] = {limits.MaxComputeWorkGroupSize[2]}");
            Logger.Warning?.PrintMsg(LogClass.Gpu, $"maxComputeSharedMemorySize = {limits.MaxComputeSharedMemorySize}");
            Logger.Warning?.PrintMsg(LogClass.Gpu, $"maxComputeWorkGroupCount[0] = {limits.MaxComputeWorkGroupCount[0]}");
            Logger.Warning?.PrintMsg(LogClass.Gpu, $"maxComputeWorkGroupCount[1] = {limits.MaxComputeWorkGroupCount[1]}");
            Logger.Warning?.PrintMsg(LogClass.Gpu, $"maxComputeWorkGroupCount[2] = {limits.MaxComputeWorkGroupCount[2]}");
            ...
            if (IsHiSilicon)
                Logger.Notice.Print(LogClass.Gpu, "GPU: HiSilicon Kirin");
        ...
        public unsafe bool RecreateSurface()
        {
            ...
            lock (SurfaceLock)
            {
                ...
                {
                    Logger.Error?.PrintMsg(LogClass.Gpu, $"RecreateSurface failed: {ex.Message}");
                    _surfaceValid = false;
                    return false;
                }
            }
        ...
        public unsafe void ReleaseSurface()
        {
            ...
            if (SurfaceLock == null || !_initialized || _window == null)
                return;
            ...
            lock (SurfaceLock)
            {
                ...
                try
                {
                    ...
                    if (_surface.Handle != 0 && SurfaceApi != null && _instance != null)
                    {
                        SurfaceApi.DestroySurface(_instance.Instance, _surface, null);
                        _surface = new SurfaceKHR(0);
                        _surfaceValid = false;
                ...
                {
                    Logger.Error?.PrintMsg(LogClass.Gpu, $"ReleaseSurface failed: {ex.Message}");
                }
                finally
                {
        ...
        public void SetPresentEnabled(bool enabled)
        {
            ...
            if (!_initialized || SurfaceLock == null || _window == null)
                return;

            if (enabled == _surfaceValid)
                return;
            ...
            lock (SurfaceLock)
            {
                ...
                if (enabled)
                {
                    if (!RecreateSurface())
                    {
                        Logger.Warning?.PrintMsg(LogClass.Gpu, "Failed to recreate Vulkan surface when enabling presentation.");
                    }
                }
                else
                {
        ...
        public unsafe void Dispose()
        {
            ...
            BackgroundCommandBufferPool?.Dispose();
            _computeCommandPool?.Dispose();
            BackgroundResources?.Dispose();
            _counters?.Dispose();
            _window?.Dispose();

            // ===== 先销毁 Pipeline（它会销毁所有 Framebuffer，Framebuffer 里引用了 dummy ImageView） =====
            _pipeline?.Dispose();

            // ===== 再销毁 HelperShader（它会销毁 dummy ImageView + Image + Memory） =====
            // 顺序不能反，否则 Pipeline.Dispose 里销毁 Framebuffer 时，
            // Framebuffer 引用的 ImageView 已经被 HelperShader.Dispose 销毁，会触发驱动报错或 assert。
