...
# ========== 2. _33 索引钳位（用 min/max 避开 Mali 的 SClamp） ==========
# 先保护数组声明，避免 [1984] 被误改
code = code.replace('shared uint _33[1984];', '@@SHARED_DECL_33@@')

def clamp_33(m):
    expr = m.group(1).strip()
    # 常量索引（如 _33[6]、_33[10]）跳过
    if expr.isdigit():
        return m.group(0)
...
# 恢复数组声明
code = code.replace('@@SHARED_DECL_33@@', 'shared uint _33[1984];')

# ========== 3. samplerBuffer 需要的扩展 ==========
